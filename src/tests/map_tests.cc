...
TEST(map_test, constr1) {
    ...
    EXPECT_EQ((*it1).second, (*it2).second); it1 = m1.begin(); for (auto it2
    = m2.begin(); it2 != m2.end(); it1++, it2++) EXPECT_EQ((*it1).first,
    (*it2).first); EXPECT_EQ(m1.size(), m2.size());
}
...
//     {11, 12}}; std::map<int, int> m22 = {{389, 12}, {3829, 12}, {111, 12},
//     {189, 12}, {11, 12}}; m1.merge(m11); m2.merge(m22); EXPECT_EQ(m1.size(),
