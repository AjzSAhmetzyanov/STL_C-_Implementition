...
class RedBlackTree {
 ...
 public:
  ...
  bool contains(const Value &key) const { return (search(key, root_) != nil_); }
  ...
  node_pointer _insert_to_node(node_pointer kekis, node_pointer new_node) {
    if (compare_.lt(*new_node->value_, *kekis->value_)) {
  ...
  size_type maxsize_() const { return alloc_.max_size(); }
